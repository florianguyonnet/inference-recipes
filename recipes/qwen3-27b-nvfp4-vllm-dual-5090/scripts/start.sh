...
PROFILE="${1:-profiles/qwen-full.env}"
if [[ -f "${PROFILE}" ]]; then
    :
...
if [[ "${YARN_OVERLAY:-1}" == "1" ]]; then
    ...
    MODEL_PATH="/root/.cache/huggingface/overlay-yarn512k"
else
    MODEL_PATH="${HF_REPO_ID:-unsloth/Qwen3.8-27B-NVFP4}"
fi
