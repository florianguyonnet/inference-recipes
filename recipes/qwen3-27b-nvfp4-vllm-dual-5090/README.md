...
- **Give another run its own HF cache.** The overlay follows `refs/main`, so
  anything that resolves `unsloth/Qwen3.8-27B-NVFP4` against the Hub in this
  cache moves that ref to the current revision, whose weights are not
  downloaded. An overlay built from it carries configs and no `*.safetensors`,
  and the container then crash-loops on `Cannot find any model weights` —
  `--restart unless-stopped` retries it forever. `make_yarn_overlay.sh` now
  refuses to build a weightless overlay; recovery is `download_model.sh`, or
  writing a complete snapshot hash back into `refs/main` (that file is written
  by the container, so it is root-owned).
